...
const COLOR_STYLES: Record<ReferralColor, { dot: string; pill: string; label: string }> = {
  RED:    { dot: "bg-red-500",    pill: "bg-red-50 text-red-700 border-red-200",       label: "Νέα — δεν ανοίχτηκε" },
  YELLOW: { dot: "bg-amber-400",  pill: "bg-amber-50 text-amber-700 border-amber-200", label: "Σε εξέλιξη" },
  GREEN:  { dot: "bg-green-500",  pill: "bg-green-50 text-green-700 border-green-200",  label: "Ολοκληρώθηκε" },
  GRAY:   { dot: "bg-slate-300",  pill: "bg-slate-100 text-slate-500 border-slate-200", label: "Πρόχειρο" },
};

type Props = {
  referral: { isDraft: boolean; openedAt?: Date | string | null; students: { status: string }[] };
  className?: string;
};
