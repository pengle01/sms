...
import { normalizeCode, randomOtp } from "@/lib/accessCode";
...
export async function startActivation(input: {
  ...
  const access = await db.studentAccessCode.findUnique({
    where: { code },
    select: { studentProfileId: true, studentClaimedAt: true },
  });
...
export async function verifyActivation(input: {
  ...
  const access = await db.studentAccessCode.findUnique({
    ...
    select: { id: true, studentClaimedAt: true },
  });
  if (!access) {
    await db.emailOtp.delete({ where: { id: row.id } }).catch(() => {});
  ...
  try {
    if (row.purpose === "ACTIVATE_STUDENT") {
      ...
      if (existing) {
        if (existing.role !== "PARENT") return { ok: false, error: "errEmailTaken" };
