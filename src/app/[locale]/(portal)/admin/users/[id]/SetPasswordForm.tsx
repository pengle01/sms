...
export function SetPasswordForm({ userId }: { userId: string }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        toast.success("Password set — the user can now sign in with email & password.");
        setPassword("");
      } else {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col sm:flex-row sm:items-end gap-3">
      <div className="flex-1">
        <label htmlFor="set-pw" className="block text-xs text-slate-500 mb-1.5">New password</label>
