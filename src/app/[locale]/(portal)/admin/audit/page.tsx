  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">{total} record{total !== 1 ? "s" : ""}{hasFilter ? " (filtered)" : ""}</p>
      </div>
      ...
      <form method="GET" className="flex flex-wrap items-end gap-3">
        ...
        <div>
          ...
          <input name="user" defaultValue={userQ} placeholder="name or email" className={selectClass} />
        </div>
        <div>
        ...
        {hasFilter && (
          <Link href="?" className="h-9 px-3 flex items-center text-sm text-slate-500 hover:text-slate-800">Clear</Link>
        )}
      </form>
          ...
          <table className="w-full text-sm min-w-[900px]">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-5 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">When</th>
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wide">Resource</th>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm text-slate-500">
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link href={pageHref(page - 1)} className="h-8 px-3 flex items-center rounded-lg border border-slate-200 hover:bg-slate-50">Previous</Link>
            )}
            {page < totalPages && (
              <Link href={pageHref(page + 1)} className="h-8 px-3 flex items-center rounded-lg border border-slate-200 hover:bg-slate-50">Next</Link>
            )}
          </div>
        </div>
      )}
